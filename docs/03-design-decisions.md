...
| D15 | 無料版の実行上限 | **最新受注10件起点のサンプル移行**: サンプル受注に紐づく商品（ハードキャップ50件）・顧客（最大10件）・受注10件のみ実インポート/エクスポート可。カテゴリ/タグは全量無料。上限はサーバーサイド（JobManager）で強制し、`cbjp/limits/*` フィルターでPro版が解除。詳細は §10.2 |
...
| 14 | 各ASP: 受注一覧の新しい順ソート指定可否（サンプル選定=D15） | F1-0 / M2-0 / B3-0 | 未 |
...
5. **フォールバック**: 受注0件のショップ・受注エンティティ未選択時は「各エンティティ最新10件」。
   受注が10件未満なら全受注 + 残枠を最新の商品・顧客で補完
