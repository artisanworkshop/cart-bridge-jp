...
当初どおり3ASP対応で設計・実装済みのため、v2.0以降は**プラットフォーム固有分岐をアダプタ外に持ち込まない**ことを検証観点とする（受注インポート時の顧客永続化フック=B4-5、JobManagerの再試行遅延拡張点=E5-1のような、プラットフォーム非依存のコア拡張点の追加自体は許容する。D18）。
...
プラットフォーム固有分岐をImporter本体に持ち込まずに動くことを確認（アーキテクチャ検証を兼ねる。旧計画ではMakeShopが担っていた観点。プラットフォーム非依存のコア拡張が必要な箇所は `04-plan-base.md` B4-5・`10-tasks.md` E5-1参照）。
