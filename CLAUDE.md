...
- `WC_Product_Variable`の`_regular_price`/`_sale_price`は`WC_Product_Variable_Data_Store_CPT::sync_price()`が保存の度に削除するため、`WC_Product_Variable::get_regular_price()`/`get_sale_price()`は常に空文字列を返す（親は`_price`にバリエーションの価格帯のみ複数値で保持する）。variable商品の価格を読む処理でこれを無条件に使うと「0円」に丸まってしまう（金銭的リスク）。代表値が必要な場合は`get_variation_price('min', false)`等バリエーション側から導出すること
