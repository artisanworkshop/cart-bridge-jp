...
- **【重大】カラーミーの都道府県コード（`pref_id`）はJIS X 0401標準・WooCommerceの都道府県番号（`WC()->countries->get_states('JP')`の`JPxx`）と並びが一致しない**（47都道府県中約20件で不一致。例: `pref_id=4`は秋田県だがJIS/Wooの4番は宮城県、16↔18福井/富山、19〜23・25〜30・31〜34・36〜37・43〜44も同様）。番号をそのまま同一視すると誤った都道府県で送受信される（`Woo\Support\AddressMapper::PREF_ID_TO_JIS_NUMBER`参照、issue #44）。この対応表はswaggerの`info.description`に**散文として埋め込まれた**「都道府県コード一覧」表（`<details>`内のMarkdownテーブル）にのみ記載されており、構造化JSONスキーマの実例（`pref_id`/`pref_name`のペア）を1件だけ見て「標準通りのはず」と一般化すると誤る（実際に一度誤った）。この種の対応表を検証する際は、JSON構造の実例だけでなくAPI説明文中の表・詳細折りたたみも確認すること
- 正規表現で「完全に指定パターンとだけ一致するか」を検証する場合、終端アンカーは`$`ではなく`\z`を使うこと。PCREの`$`は末尾改行の直前にもマッチするため、`^[0-9\-]+$`のような形式チェックは末尾に`\n`が混入した値（例: `"0312345678\n"`）を誤って「一致」と通してしまう
- カラーミーの`address1`は「市区町村・番地」を1フィールドに含むが、WooCommerceのJPロケール（`WC()->countries->get_address_fields('JP')`）は`billing_city`（市区町村）と`billing_address_1`（番地）を別の必須フィールドとして扱う。エクスポート方向でColorMeへ住所を送る際は`city`+`address_1`の連結が必要（`Adapters\ColorMe\Transform\CustomerTransformer::join_address1()`参照）。`push_order()`実装時も同じ連結が必要になる
