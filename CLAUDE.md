...
- WPテストスイートは各テストクラスの終了時（`tear_down_after_class()` → `_delete_all_data()`）に、term_id=1 以外の全ターム・投稿・ユーザーをトランザクション外で削除して commit する（**options は消さない**）。bootstrap の `WC_Install::install()` が作る既定カテゴリ「Uncategorized」（`default_product_cat`=15）は**プロセス最初のテストクラスの間だけ有効**で、2クラス目以降は option が消えたタームを指したままになる（`update_terms()` の自動付与は存在しない int ID を `wp_set_post_terms()` が読み飛ばして no-op）。CI（デフォルト順）は先頭が `AbstractPlatformAdapterTest` なので常に「消えた」状態で通り、ランダム順で先頭クラスになったときだけ落ちる（issue #63）。既定カテゴリの有無に依存するテストは `default_product_cat` を自分で明示すること（有効なタームを指すか `0`）。この種の順序依存は `phpunit <テストファイル>` で1クラスずつ単独実行すると（＝各クラスが先頭になる）決定的に洗い出せる。全体を `--order-by=random` で回しても先頭クラスになる確率は低い（ネストしたスイート単位でシャッフルされる）
- フェイルクローズ分岐のテストは、入力が**本当にその分岐に入るか**（別の分岐でも同じ結果にならないか）を確認し、分岐を一時的に壊して落ちること（ミューテーション）まで確かめる。「税設定不能の店舗で実売価格だけ換算不能」のテストは通常価格も換算不能になり、狙った分岐を通らないトートロジーだった（独立レビューで検出。PR #61 R1-1）
