...
- `JobManager::start_run()` は `has_active_job_for_platform()` で同一プラットフォームの同時実行を防ぐが、`JobManager::retry()` は当初このガードを経由せず、対象ジョブが `failed` であることしか見ずにrequeueしていた（E2-4 PR #53 G2/G3、Codex/Copilot指摘。issue #54）。`has_active_job_for_platform()` をそのまま `retry()` に足すだけでは不十分な点に注意: `start_run()` は対象runの全エンティティジョブを最初に `pending` で作成するため、失敗した1ジョブの**同じrunの兄弟ジョブ**まで「進行中」として誤検知し、正当な同一run内のRetryまでブロックしてしまう。対応として `JobRepository::has_active_job_for_platform_excluding_run( $platform, $run_id )` を新設し、対象ジョブ自身の `run_id` を判定から除外した（`JobManager::retry()` が異なる run のアクティブジョブを検知した場合は `start_run()` と同じ `RunAlreadyInProgressException` を投げ、`RestController::retry_job()` は既存の `run_in_progress_error()`（409）で応答する）。**同種の「`start_run()` が持つ同時実行ガードを他の状態遷移操作が共有していない」問題は `cancel_run()` にも残っている**（`f1-6-import-ui/R1-X1`。Action Scheduler側で実行中の`process_job()`を中断しないタイミング競合で、根本原因が異なるため本修正のスコープには含めていない）
